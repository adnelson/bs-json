...
let pair = (decodeA, decodeB, json) =>
  if Js.Array.isArray(json) {
    ...
    if length == 2 {
      ...
      \"@@"(
        raise,
        DecodeError("Expected array of length 2, got array of length " ++ string_of_int(length)),
      )
    }
  } else {
    raise(DecodeError("Expected array, got " ++ _stringify(json)))
...
let tuple3 = (decodeA, decodeB, decodeC, json) =>
  if Js.Array.isArray(json) {
    ...
    if length == 3 {
      ...
      \"@@"(
        raise,
        DecodeError("Expected array of length 3, got array of length " ++ string_of_int(length)),
      )
    }
  } else {
    raise(DecodeError("Expected array, got " ++ _stringify(json)))
...
let tuple4 = (decodeA, decodeB, decodeC, decodeD, json) =>
  if Js.Array.isArray(json) {
    ...
    if length == 4 {
      ...
      raise(
        DecodeError("Expected array of length 4, got array of length " ++ string_of_int(length)),
      )
    }
  } else {
    raise(DecodeError("Expected array, got " ++ _stringify(json)))
    ...
    | list{} =>
      let formattedErrors = "\n- " ++ Js.Array.joinWith("\n- ", Array.of_list(List.rev(errors)))
        ...
        DecodeError(
          ...
          (formattedErrors ++
          ("\nAnd the JSON being decoded: " ++ _stringify(json))),
        ),
      )
    | list{decode, ...rest} =>
